...
- [ ] **W/L impact sign should track win prob, not the pitcher's record.** `projection.py` computes `w_contrib = raw_w × win_prob × STARTER_WIN_SHARE × 5` and `l_contrib = raw_l × (1−win_prob) × … × (−5)`, where `raw_w`/`raw_l` are the pitcher's historical W/L *rates*. So the **sign** of the W/L impact depends on the pitcher's record, not the matchup — a losing-record pitcher shows a *negative* W/L impact even when favored (observed in the tooltip: 56% win prob → −0.1 pts). Fix: scale both contributions by the decision rate `(raw_w + raw_l)` split by win prob, so net = `(raw_w+raw_l) × STARTER_WIN_SHARE × 5 × (2·win_prob − 1)` → **positive when favored, 0 at a coin flip**, magnitude still pitcher-specific. Mirror in the lock path; changes projections, so add a `MaeTimelineChart` milestone marker.
- [ ] **ESPN Forecaster scraper broken since ~mid-May** — the accuracy dashboard has no ESPN projection data after ~May 17 (orange line flatlines while Skipper continues). No code changed Apr 27–Jun 5, so it's an *external* break (ESPN changed/blocked the Forecaster page; likely the stale `Chrome/123` User-Agent in `api/forecaster.py:HEADERS`, the `table.inline-table` selector, or the article URL). The Forecaster is a rolling 10-day window, so the missed weeks are **unrecoverable** — fixing the scraper resumes data going forward. **Next step:** hit `/api/forecaster` (auth-exempt diagnostic) to see the current failure mode (error / empty `entries` / changed HTML), then fix `forecaster.py` accordingly. Also add the ESPN-lock counters to `cache:cron-summary:{date}` so a future break is visible without log access.
