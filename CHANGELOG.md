...
- **New surfaces expose old assumptions.** This is the recurring pattern of the season — session 27's UTC-vs-ET trigger bug, session 25's `actual-all:` partial-write silent failure, and now session 28's rostered `percentOwned` default-to-100. Each was code that had been silently wrong for a while because nothing was reading the value. The Stats tab's Own% column was the first place rostered ownership was displayed, and immediately surfaced the bug. The architecture-decisions takeaway: when adding any new UI surface that displays a previously-unsurfaced field, audit the upstream extraction once before shipping. KNOWLEDGE.md now lists this as an explicit checklist item.
- **Fold small bug fixes into the originating PR when the diff has a clean place for them.** PR #117 shipped a feature PR + a 5-line backend fix that the feature PR exposed. One commit, one PR, one verify cycle, no broken-UI window in production. The alternative (merge feature, ship broken-looking column, file follow-up PR) would have created a multi-deploy-cycle gap. The rule that crystallized: if the originating PR's diff has a clean place for the fix and the fix is small, fold; if either property fails, ship broken UI and patch.
- **`vercel --prod` deploys from the local working tree without requiring a `git push`.** This is a footgun. PR 2's code was running in production hours before any commit existed in git; PR 3's tooltip edits were then layered onto a working tree that didn't reflect any pushed branch. Recovery was bundling PR 2 + PR 3 into a single PR (rather than trying to retroactively split the diff). The workflow rule going forward: always push the branch before running `vercel --prod`, or treat `--prod` deploys without a corresponding git commit as ephemeral and re-run after the push lands. KNOWLEDGE.md → Development Workflow documents this.
- **Bundle PRs when their changes overlap in the same file.** Splitting `components/StatsTable.tsx` changes between a "columns" PR and a "tooltips" PR would have required `git add -p` interactive staging — too error-prone for the workflow this project uses. Cleaner rule: when two pieces of work touch the same file, ship them in one PR even if they're conceptually separate. Preserve the conceptual story in the PR description, not the commit boundary. Combined PR titles (e.g. `feat: Stats tab season + Savant columns with header tooltips`) are fine and read better than splitting.
- **Native HTML `title` attribute is a perfectly good v1 for column header help text.** The choice between custom-styled tooltip components (matching the app's dark theme) and the browser-native `title` attribute came down to: do we need rich content per tooltip? For glossary lookups on column headers, no — one sentence is enough. The native attribute ships in ~30 lines (interface field + render-time pass-through), is accessible to screen readers without ARIA wiring, and degrades gracefully on every browser. Reserve the styled-tooltip pattern for places that need rich content (the existing `<ProjectionTooltip>` is the right shape there).
- **"Data we have" vs. "data the BACKLOG specified" — verify before scoping.** The session-21 Stats-tab design listed Whiff% as a v1 column. Implementation revealed Whiff% wasn't in `fetch_statcast_stats()` at all — it lives on `fetch_pitch_arsenal()`, which the Skipper doesn't currently fetch. Shipped the column with a tooltip noting "data plumbing in progress" rather than expanding scope to wire up arsenal fetching mid-PR. Filed for follow-up. The discipline: when picking up old design notes, confirm what's actually retrievable from existing data sources before estimating scope.
- **Config-driven column system pays off immediately.** The `PITCHER_COLUMNS` array pattern was specified in the BACKLOG primarily to make a future hitter expansion cheap. It also paid off within session 28 itself — adding the 9 PR #118 columns required no changes to the table renderer, just new entries in the array. New `preferredDir` and `tooltip` fields slotted in without any column-by-column code edits. Config-driven systems compound across PRs that touch the same surface.

### PR #117 — Stats tab scaffolding + latent percentOwned bug fix

Tab toggle (Schedule / Stats) on the My Team SP card. New `<StatsTable>` component with config-driven `PITCHER_COLUMNS` array — table is renderer-only, all data shaping in column defs. v1 columns from data already on the API response: Pitcher, Team, Slot, Own%, Starts, FPTS/G, Proj FPTS, Act FPTS. Sortable by any column; clicking the same header twice flips direction; numeric columns default desc, string columns default asc. IL pitchers always sink to the bottom regardless of sort direction (mirrors the existing ScheduleGrid IL placement). Act FPTS row total excludes pre-acquisition dates (the rostered-window invariant from PRs #81 / #111 / #115).

Mid-PR, the new Own% column rendered "100%" for every rostered player. Investigation: `api/espn.py` line 317 was reading `pool_entry.get("percentOwned", 100)` from the `mRoster` response — a field that doesn't exist there — and falling through to the default of 100. The free-agent path at line 424 reads from the right place (`player.ownership.percentOwned` on the `kona_player_info` response) and has always worked. The bug was silent because rostered ownership had never been displayed before; the Stats tab's Own% column was the first surface that surfaced the value.

Fix folded into the same PR: extended the existing `kona_player_info` parsing loop (which was already capturing `proj_fpts_by_player` from the same response) to also capture `player.ownership.percentOwned` into a `percent_owned_by_player_id` dict, then sourced rostered `percentOwned` from that dict. Zero new HTTP calls. The fix sits next to the projection-fpts capture, not in a new helper, because both pluck-from-kona-player-info reads belong together.

Diff: `components/StatsTable.tsx` (new file, ~250 lines), `pages/my-team.tsx` (+84/-17), `api/espn.py` (~10 lines for the percentOwned fix).

### PR #118 — Stats tab season + Savant columns with header tooltips (combined PR 2 + PR 3)

Nine new columns inserted between Starts and FPTS/G: W-L (combined cell), ERA, K/9, BB/9, xERA, xwOBA, wOBAΔ, Brl%, Whiff%. Total Stats tab is now 17 columns; horizontal scroll on smaller screens. The PR was originally scoped as two — PR 2 (columns) and PR 3 (header tooltips) — but ended up combined because both touched `components/StatsTable.tsx` and the workflow trap (see workflow notes below) made splitting them retroactively impractical.

**Backend (`api/fetcher.py`):** New `cache:savant-statcast:{year_int}` cache entry with 24hr TTL, populated from `savant.fetch_statcast_stats()`. Mirrors the existing Savant expected-stats caching pattern; isolated key so a Statcast outage can't poison the projection-driving `cache:savant:{year}` entry. `load_cached_data()` now returns `savant_statcast_current` alongside the existing `savant_current` and `savant_previous`.

**Backend (`api/espn.py`):** Two new helpers `_build_season_stats()` and `_build_savant_expected()` produce compact, frontend-friendly sub-dicts with K/9 and BB/9 precomputed on the backend. Each helper returns `None` when there's nothing meaningful to display (no IP, or no Savant data) so the frontend renders an em-dash without per-field null checks. Each `rosterSPs` / `freeAgents` / `droppedPlayers` entry now carries `seasonStats` and `savantExpected`. Three identical-shape attach blocks because they live in three independent loops; refactoring them into a single helper would have required reshaping the input lists.

**Frontend (`components/StatsTable.tsx`):**
- New `preferredDir?: 'asc' | 'desc'` field on the `PitcherColumn` interface. Columns where lower is better (ERA, BB/9, xERA, xwOBA, Brl%) declare `preferredDir: 'asc'` so a single click puts the best pitchers at the top. The sort comparator honors this on first click; subsequent clicks of the same column flip direction normally.
- Sort comparator now sinks NaN sortValues to the bottom regardless of direction. Pitchers without season stats (rookies, fresh call-ups) or without Savant footprints stay at the bottom whether the column is sorted asc or desc. Mirrors the existing IL-sinks-to-bottom behavior; both are direction-independent.
- New `tooltip?: string` field on `PitcherColumn`, rendered via the native HTML `title` attribute on each `<th>`. Tooltips skip self-explanatory columns (Pitcher, Team) and explain everything else. wOBAΔ and Whiff% tooltips include interpretation guidance ("positive = pitcher has been unlucky..."), Slot decodes the four short codes, Act FPTS calls out the pre-acquisition exclusion.
- New formatting helpers: `fmtBaseballDecimal()` strips the leading zero (".285" not "0.285"); `fmtWobaDiff()` adds an explicit `+` sign for non-negative values so the green/red color coding is reinforced by the sign. New `<NumOrDash>` wrapper component renders an em-dash for any missing numeric value (consolidates a pattern that recurs across every season/Savant column).

**Known data gaps shipped with the PR:**
- Whiff% column is empty for every pitcher. `fetch_statcast_stats()` doesn't include `whiff_pct` — that field lives on `fetch_pitch_arsenal()` (per-pitch-type), which the Skipper doesn't currently call. Tooltip notes "data plumbing in progress." Filed as a follow-up.
- Dylan Cease specifically missing Brl% while every other rostered SP has a value. Almost certainly a name-match issue between the Savant CSV and our `strip_accents` lookup. Worth a one-off diagnostic print of unmatched names. Filed as a follow-up.

Diff: `api/espn.py` (+156/-42), `api/fetcher.py` (+45 mixed), `components/StatsTable.tsx` (+184 mixed). Three files.

### Decisions and architecture

- **Config-driven column system established.** `PITCHER_COLUMNS` is now a real surface — 17 entries, each with optional `sortValue` / `stringValue` / `preferredDir` / `tooltip` / `align` / `minWidth` / `render`. Adding a column is editing one array. Future PRs (luck badge, projected pace, FA tab, hitter expansion) plug in here.
- **Statcast cache is isolated from the Savant projection cache.** `cache:savant-statcast:{year}` lives separately from `cache:savant:{year}` despite both being Savant-sourced. Statcast (Brl%, Whiff%, etc.) is only used for display today; expected stats (xERA, xwOBA) feed the projection model. Isolating them means a Statcast outage can't poison projections.
- **Native `title` for column tooltips, custom component for cell tooltips.** Two patterns coexist — the existing `<ProjectionTooltip>` (rich, portal-rendered) for cells that need detailed breakdown content, and native `title` for headers that need glossary lookups. Worth keeping this distinction explicit so future PRs reach for the right one.
- **New backlog items logged:** Whiff% data plumbing, Dylan Cease Brl% name-match, Free Agents Stats tab, Relievers Stats tab. Luck badge and Projected season pace remain in the Stats-tab follow-up section as previously scoped.
- **Backlog item closed:** "Stats view tab on My Team / Free Agents" (the umbrella session-25 design item) — scaffolding, season, Savant, and tooltip pieces all shipped. Sub-items above are the residue.

### Workflow notes

- **`vercel --prod` from an unpushed working tree.** The session's biggest workflow snag. Sequence: PR 2's code was edited locally → Conner ran `vercel --prod` to verify in production (showed me the screenshot) → I assumed PR 2 was merged from the screenshot evidence and started PR 3 tooltip work → Conner mentioned PR 2 was never `git push`-ed → PR 2 + PR 3 were now both uncommitted in the same working tree. Recovery: bundle into a single PR, single commit. The workflow rule going forward: always push before running `--prod`, or treat unpushed `--prod` deploys as ephemeral verification only. Documented in KNOWLEDGE.md.
- **Mid-session, a stage step missed PR 3's tooltip changes from the original combined commit.** After Conner's first commit + push on the combined branch, `git status` showed `components/StatsTable.tsx` still modified — turned out the tooltip edits weren't in the working tree at the moment of `git add` (likely a transient editor/formatter state). Resolved with a follow-up commit on the same branch (`git add` → `git commit` → `git push` → PR picks it up automatically). No rework, no force-push. The general pattern: when you discover post-push that something didn't make the commit, add a follow-up commit on the same branch rather than amending; the open PR collects all commits and squash-merge collapses them.
- **Sandbox validation rules from sessions 24/25 held.** Python validation via `python3 -c "import ast; ast.parse(...)"` only. TypeScript validation via `npx tsc --noEmit`. Zero `.git/` lock contention all session.
- **One slipped-up `git diff --stat` from the sandbox** — caught immediately, no `.git/index.lock` left behind, but worth re-stating: any git invocation including read-only inspection runs in Conner's terminal, never the sandbox. The session-25 rule is unchanged.
...
- **"Tag, don't drop" beats "filter and lose data".** PR #111's choice between dropping pre-acquisition `startDates` from the data flow vs tagging them with `preAcquisition: true` came down to one product question: do we want the schedule grid to keep showing the cell? Backlog said yes ("informative — what we missed"), so tagging won. Tagging also kept the JSON payload backward-compatible — old frontend code (or a bookmarked tab mid-deploy) ignores the new field and behaves as it did before. Default to tagging when the data is informative; reserve dropping for data the frontend has no use for.
- **Post-hoc adjustment is a legitimate alternative to a reorder, when the cost of being wrong is bounded.** The cleanest fix for PR #111 was to move `get_actual_fpts()` above `get_projected_fpts()` so tagging happens before projection runs — which would have cleanly excluded pre-acq starts from the locked `proj2:` projections too. But that reorder requires also moving the FA HTTP fetch up (since actuals needs FA names), changing roughly the top half of `get_league_data()`. The post-hoc approach (tag after the fact, recompute totals from `per_start_details`) was a contained ~65-line addition with no reorder. The price paid: the lock-skip fix is forward-only deferred to a follow-up. Tradeoff was worth it because the lock-skip's user-visible impact is narrow (already-locked projections from cron or prior owners exist anyway) and the reorder risk was real.
- **Forward-only fixes need explicit scope notes in BACKLOG.** PR #111 had three obvious follow-ups that would have ballooned the PR if pulled in. Each one got logged to BACKLOG with a one-line "why deferring is safe" rationale. The discipline: if you can name *what* the follow-up is and *why deferring doesn't hurt anyone visible today*, the deferral is responsible. If you can't name either, you probably need to expand the PR.
- **Pre-existing bugs surface as a side effect of new feature work, not because the new feature broke anything.** The "Montero's start isn't showing up on My Team" report came right after PR #111 deployed. Initial worry: did we just regress something? Quick trace through the new pre-acquisition code (early-return on empty `days_on_team_set`) ruled it out — the bug had been there since session 18 PR #73's transaction-lag fix, but only became visible when someone happened to add a pitcher during the narrow window where the current-period roster was locked but the next-period roster reflected the addition. Lesson: when a "new" bug surfaces right after a deploy, don't assume the deploy caused it; trace the code path with the specific input.
- **A deployed PR's regression test is the next user action.** PR #111's verification checklist had us looking for Wrobleski's pre-acq cell in the schedule grid (rendered correctly, gray with em-dash, opaque tooltip on hover). The regression we missed was Montero — but Montero hadn't been added at the time the checklist ran. Real-world workflow surface area is what catches the next class of bugs, not pre-deploy theorycrafting. Build verification checklists tightly around what the PR changed; let the next user action surface the rest.
- **Documenting an architecture pattern beats documenting a single bug fix.** KNOWLEDGE.md gains a "Post-hoc adjustment vs data-flow reorder" architecture decision in this session, not a "PR #111 fixed Wrobleski's case" note. The decision pattern recurs (PR #81 was the same shape; future scope-creep moments will be too); the bug specifics don't. Pattern-first documentation pays off across sessions.
- **Diagnostic clarity ≠ implementation completeness.** Montero's bug got fully traced this session (root cause: derived structures stale after lag refetch, contained-fix scope, ~10 line addition). The temptation to ship it as a same-session second PR was real, but the discipline of "one PR, fully verified, then stop" was more important than rolling diagnoses into shipped code. Logging a diagnosed-but-unfixed bug to BACKLOG with a complete scope sketch leaves the next session well-loaded; pretending the diagnosis was the fix would have been worse than shipping nothing.

### PR #111 — Roster-window intersection for currently-rostered pitchers

Four files, 177 insertions / 6 deletions. Diff was forward-only (almost all additions, almost no deletions) because the change is "add new branches and tag a new field" rather than "rewrite existing logic."

- `api/espn.py` — new post-hoc adjustment block inserted right after `roster_actual_fpts` is built (right before the dropped-player branch). For each pitcher in `roster_sps`:
  1. Compute `days_on_team_set = {d for d in my_team_pitchers_by_day if name in players}` — the set of dates the pitcher actually appeared on our roster within the matchup window.
  2. Walk `player["startDates"]`; tag any entry with `date <= today_iso AND date NOT IN days_on_team_set` with `preAcquisition: True`.
  3. If any starts got tagged: recompute `player["starts"]` as the count of non-pre-acq entries; subtract the sum of pre-acq starts' `proj` values from `player["projFpts"]` and from `breakdown["total"]`; propagate the flag onto `proj_details_roster[name]["starts"]` entries so the tooltip can render the pre-acq variant.
  4. Print one log line per affected pitcher: `[espn.py] Pre-acquisition: <name> — tagged N start(s) ([dates]); effective starts=X, projFpts=Y`. Future regressions surface in Vercel logs at deploy time.
- `components/ProjectionTooltip.tsx` — `StartDetail` interface extends with optional `preAcquisition?: boolean`. New branch in start-mode: when `startDetail.preAcquisition` is true, render a different tooltip body — header `"Pre-acquisition · @COL"`, explanation text `"This start happened before you picked up this pitcher. It's shown for context but isn't counted toward your totals."`, and a dim `"Would have projected"` row at the bottom. Same opaque dark-background portal styling as the existing tooltip — Conner explicitly called out he wanted it to match the other tooltips, not use a browser-default `title` attribute.
- `components/ScheduleGrid.tsx` — new branch in `DayCell` past/today path that fires when `isStarting && startInfo.preAcquisition`. Renders the cell in `var(--ink-3)` muted gray throughout: gray opp label, em-dash indicator instead of green ✓, gray FPTS (no green/red sign coloring), wrapped in `ProjectionTooltip` so the explanation surfaces on hover. The Act FPTS row total in the table footer now subtracts pre-acq dates: builds `preAcqDates: Set<string>` from `pitcher.startDates.filter(s => s.preAcquisition)`, then sums `actualFpts[name][date]` only for entries whose date is NOT in the set.
- `pages/my-team.tsx` — `actual` aggregate filter changed from `s.date <= today` to `s.date <= today && !s.preAcquisition`. `projected` aggregate is unchanged because it reads `p.starts`, which the backend already corrected.

Production verification post-deploy: Wrobleski's row showed muted Apr 20 @COL cell with em-dash and `(+12.4)` in gray; Projected Starts tile showed effective count; ProjectionTooltip on hover rendered the pre-acquisition variant with opaque background.

### Diagnosed but not fixed: transaction-lag refetch leaves `starts_map` stale (Montero case)

Pre-existing bug from session 18 PR #73's transaction-lag block. The lag fix updates `roster_entries` from the next-period mRoster but leaves the derived structures (`all_player_names`, `roster_team_map`, `starts_map`) computed from the original first-fetch player list. Players added during the locked window land in the new `roster_entries` but fall through `starts_map.get(name, {})` to the empty default, rendering as a row with `starts=0`, `startDates=[]`, all-gray cells, no projection.

The diagnostic was straightforward once the symptom was named: free-agent view shows Montero's @CIN start because that surface uses its own independent `get_starts_for_players` call against `fa_names`; the My Team surface uses the rostered `starts_map`, which is stale. The proposed fix is a small addition inside the existing `if today_has_started(schedule):` branch — after the refetch succeeds and `roster_entries` is updated, re-extract `all_player_names` and `roster_team_map` from the refreshed entries and re-call `get_starts_for_players` to rebuild `starts_map`. One extra MLB Stats API call only on the codepath where games are already in progress and the refetch fired.

Why it took until session 26 to surface: the bug requires a player to be on the next-period roster but NOT the current-period roster. That window is narrow (most adds also reflect on the current period), and it only matters during the locked-period codepath. Conner happened to add Montero at the right moment, post-deploy.

Why it didn't get fixed in the same session: discipline of "one PR, fully verified, then stop." The fix is small and contained but needed its own diff, its own commit, and its own verification path. Logged to BACKLOG with a complete implementation sketch so the next session opens warm.

### Decisions deferred (logged to BACKLOG)

Three follow-ups that PR #111 deliberately did not pull in. Each has a one-line rationale in BACKLOG explaining why deferral is forward-only safe:

1. **`projection.py` lock-skip for pre-acquisition starts.** Forward-fix needs the actuals fetch above projection; substantial reorder. Practical impact today is low because most pre-acq dates already have `proj2:` locks from cron or prior owners.
2. **Accuracy dashboard "My Roster" scope filter for pre-acq.** Even after lock-skip, legacy `proj2:` keys from before tagging existed will still match. Symmetric fix to PR #104's FA-leak filter. Should ride alongside the lock-skip PR.
3. **Dropped-player post-drop Act FPTS pruning.** Same intersection bug shape PR #111 fixed for rostered players, applied to the dropped-player branch's unfiltered `info["player_fpts"]`. Small visible impact (most dropped pitchers don't accrue FPTS post-drop) but symmetric and easy to fix.

### Workflow notes

- **TypeScript checking continues to flag a pre-existing recharts type drift in `MaeTimelineChart.tsx`.** Not introduced by PR #111. Logged here for traceability; not blocking deploys.
- **Sandbox validation rules from session 25 held.** All Python validation done via `python3 -c "import ast; ast.parse(...)"`. All git ops in Conner's terminal. Zero `.git/` lock contention this session.
- **Asking-the-user discipline applied late but applied.** Mid-session, Claude's docs assumed a Montero fix had shipped as a separate PR (based on a misread of "I already committed PR 111"). Conner called it out; Claude asked targeted questions and corrected. Earlier asking would have prevented the inaccuracy entirely. Pattern: when the model is hedging in writing ("if your fix differs..." / "based on diagnosis..."), that's a signal to pause and ask, not a signal to ship hedged docs.
